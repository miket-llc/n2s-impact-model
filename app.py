...
def create_sidebar_controls():
    ...
    **Your Current Maturity: Level {current_maturity['maturity_level']} - {current_maturity['maturity_name']}**
    ...
    cicd_maturity_idx = 0
    cicd_response = assessment_responses.get(
        'ci_cd_maturity', 'Manual deployments')
    ...
    cicd_pct = (cicd_maturity_idx / (len(cicd_options) - 1)) * \
        100 if len(cicd_options) > 1 else 0

    code_reuse = assessment_responses.get('code_reuse_level', 20)

    env_auto_idx = 0
    env_response = assessment_responses.get(
        'environment_automation', 'Manual environment setup')
    ...
    env_pct = (env_auto_idx / (len(env_options) - 1)) * \
        100 if len(env_options) > 1 else 0

    doc_auto_idx = 0
    doc_response = assessment_responses.get(
        'documentation_automation', 'Manual documentation')
    ...
    doc_pct = (doc_auto_idx / (len(doc_options) - 1)) * \
        100 if len(doc_options) > 1 else 0

    # Calculate average assessment maturity
    avg_assessment_maturity = (
        test_coverage + cicd_pct + code_reuse + env_pct + doc_pct) / 5
    ...
    if 'initialized_target' not in st.session_state:
        ...
        st.session_state.default_target = min(
            20, int(current_maturity['current_savings_potential']))
    ...
    # We'll need selected initiatives for feasibility check - get a preview
    # first
    ...
    if feasibility['feasible']:
        st.sidebar.success(
            f"✓ Target {target_savings}% is achievable with your current maturity")
    else:
    ...
    scenario_config = {
        ...
        'description': f"Target {target_savings}% savings (Current maturity: Level {
    ...
    # No need for user to adjust - matrix already at 75th percentile of
    # research
    ...
    with st.sidebar.expander("🔍 Debug: Current State", expanded=False):
        ...
        st.write(
            f"Maturity Potential: {
                current_maturity['current_savings_potential']:.1f}%")
...
**High Risk Example:** {phase_info['high_risk']}"""
    ...
    # Initiative Maturity - DIRECTLY SET BY ASSESSMENT (with manual override
    # option)
...
def display_kpi_metrics(kpi_summary):
    ...
    with col4:
        st.metric(
            ...
            format_currency(
                kpi_summary['total_financial_benefit']),
...
def create_hours_breakdown_by_phase_chart(summary_df, cost_results):
    ...
    hours_saved = summary_df['Hour Variance'].tolist()
    ...
    for phase in phases:
        if phase == 'Post Go-Live':
            ...
            blended_rate = cost_results['baseline_cost'][phase] / baseline_hours[phases.tolist(
            ).index(phase)] if baseline_hours[phases.tolist().index(phase)] > 0 else 100
            hours_avoided.append(
                cost_avoidance /
                blended_rate if blended_rate > 0 else 0)
        else:
...
def create_hours_saved_chart(summary_df):
    ...
    colors = ['green' if x < 0 else 'red' if x >
              0 else 'gray' for x in hours_variance]
...
def create_initiative_impact_chart(initiative_df):
    ...
    # Reverse the order so best initiatives (most negative/savings) appear at
    # top
    ...
    fig.add_trace(
        go.Bar(
            ...
            text=[
                format_currency(
                    abs(x)) for x in significant_df['Total Financial Impact']],
            textposition='outside'))
...
def export_to_excel(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if initiative_impact_df is not None:
            initiative_impact_df.to_excel(
                writer, sheet_name='Initiative Impacts', index=False)
    ...
    with st.expander("Maturity Analysis & Recommendations", expanded=False):
        ...
        **Description:** {current_maturity['maturity_description']}
        ...
        for area, description in current_maturity['automation_characteristics'].items(
            ...
            st.markdown(
                f"- **{area.replace('_', ' ').title()}**: {description}")
        ...
        if not feasibility['feasible']:
            ...
            **Required Maturity Level:** {feasibility['required_maturity_name']} (Level {feasibility['required_maturity_level']})
            ...
            for i, recommendation in enumerate(
                    feasibility['recommendations'], 1):
                st.markdown(f"{i}. {recommendation}")

        else:
            st.success(
                "Your current maturity level supports your target savings goal!")

    # Run calculations with NEW INDUSTRY-GROUNDED MODEL
    try:
        # Use new simplified model that calculates savings directly from
        # initiative maturity
        ...
        with st.expander("🔬 Calculation Trace (Debug)", expanded=False):
            ...
            st.write(
                f"Maturity Level: {
                    controls['current_maturity']['maturity_level']}")
            st.write("")
            ...
            st.write(
                "**NOTE:** Change the initiative sliders below to see savings update!")
            st.write("")
            ...
            if actual_savings_pct < target_savings:
                ...
                st.write(
                    f"💡 Try: Increase initiative maturity levels above {max([m for m in controls['maturity_levels'].values()])}%")
        ...
        cost_breakdown_chart = create_cost_breakdown_by_phase_chart(
            cost_results, summary_df)
        ...
        **Key for Executives:**
        ...
        hours_breakdown_chart = create_hours_breakdown_by_phase_chart(
            summary_df, cost_results)
        ...
        **Key for Executives (Hours):**
        ...
        st.markdown(
            "**Data used to create the Executive Hours Summary chart above:**")

        # Create the data table for hours chart using available data
        hours_chart_data = []
        for phase in summary_df['Phase']:
            baseline_hours_val = summary_df[summary_df['Phase']
                                            == phase]['Baseline Hours'].iloc[0]
            modeled_hours_val = summary_df[summary_df['Phase']
                                           == phase]['Modeled Hours'].iloc[0]
            hours_saved_val = summary_df[summary_df['Phase']
                                         == phase]['Hour Variance'].iloc[0]

            # Calculate hours avoided for this phase
            if phase == 'Post Go-Live':
                cost_avoidance = cost_results['avoidance'][phase]
                blended_rate = cost_results['baseline_cost'][phase] / \
                    baseline_hours_val if baseline_hours_val > 0 else 100
        ...
        if initiative_chart:
            ...
            st.info(
                "No significant initiative impacts to display (adjust maturity levels to see impacts)")
        ...
        **Role allocation shows how hours and costs are distributed across team members**
        ...
        # Calculate role-specific multipliers based on initiative maturity
        # levels
        role_multipliers = calculate_role_initiative_multipliers(
            controls['maturity_levels'])
        ...
        # Modeled: apply role-specific multipliers so different roles benefit
        # differently
        ...
        role_summary_df = get_role_summary(
            baseline_role_hours, modeled_role_hours)

        # Display role group totals
        col1, col2, col3 = st.columns(3)

        baseline_group_totals = calculate_role_group_totals(
            baseline_role_hours)
        modeled_group_totals = calculate_role_group_totals(modeled_role_hours)

        with col1:
            st.metric(
                "Pod Team Hours (Baseline)", f"{
                    baseline_group_totals['Pod']:,.0f}", f"{
                    modeled_group_totals['Pod'] - baseline_group_totals['Pod']:,.0f}")

        with col2:
            st.metric(
                "Pooled Team Hours (Baseline)", f"{
                    baseline_group_totals['Pooled']:,.0f}", f"{
                    modeled_group_totals['Pooled'] - baseline_group_totals['Pooled']:,.0f}")
        ...
        display_role_summary['Baseline Hours'] = display_role_summary['Baseline Hours'].apply(
            lambda x: f"{x:,.0f}")
        display_role_summary['Modeled Hours'] = display_role_summary['Modeled Hours'].apply(
            lambda x: f"{x:,.0f}")
        display_role_summary['Hours Saved'] = display_role_summary['Hours Saved'].apply(
            lambda x: f"{x:,.0f}")
        display_role_summary['% Saved'] = display_role_summary['% Saved'].apply(
            lambda x: f"{x:.1f}%")
        display_role_summary['Hourly Rate'] = display_role_summary['Hourly Rate'].apply(
            lambda x: f"${x:,.0f}")
        display_role_summary['Cost Savings'] = display_role_summary['Cost Savings'].apply(
            lambda x: f"${x:,.0f}")
        ...
        baseline_costs_by_role = baseline_role_costs.sum(
            axis=1).sort_values(ascending=True)
        modeled_costs_by_role = modeled_role_costs.sum(
            axis=1).loc[baseline_costs_by_role.index]
        ...
        **Savings categorized by strategic value drivers**
        ...
        display_category_df['Total Hours Saved'] = display_category_df['Total Hours Saved'].apply(
            lambda x: f"{x:,.0f}")
        display_category_df['Total % Saved'] = display_category_df['Total % Saved'].apply(
            lambda x: f"{x:.1f}%")
        display_category_df['OOtB Config Hours'] = display_category_df['OOtB Config Hours'].apply(
            lambda x: f"{x:,.0f}")
        display_category_df['Methodology Hours'] = display_category_df['Methodology Hours'].apply(
            lambda x: f"{x:,.0f}")
        display_category_df['AI & Automation Hours'] = display_category_df['AI & Automation Hours'].apply(
            lambda x: f"{x:,.0f}")
        display_category_df['OOtB Config $'] = display_category_df['OOtB Config $'].apply(
            lambda x: f"${x:,.0f}")
        display_category_df['Methodology $'] = display_category_df['Methodology $'].apply(
            lambda x: f"${x:,.0f}")
        display_category_df['AI & Automation $'] = display_category_df['AI & Automation $'].apply(
            lambda x: f"${x:,.0f}")
        display_category_df['Total Cost Saved'] = display_category_df['Total Cost Saved'].apply(
            lambda x: f"${x:,.0f}")
        ...
        with col3:
            ...
            model_params = {
                ...
                'model_results': {
                    'actual_savings_percentage': abs(
                        kpi_summary['total_hours_saved_pct']),
        ...
        with st.expander("Risk Assessment Guide"):
            ...
            for phase in PHASE_ORDER:
                ...
                st.markdown(
                    f"**High Risk Example:** {phase_info['high_risk']}")
                st.markdown("---")
